...
def Tower_abnormal(df):
    ...
    for i in range(df.shape[0]):
        condition1 = df.iloc[i,2] < hp_1 and df.iloc[i,3] > hp_2
        condition2 = df.iloc[i,2] > hp_3 and df.iloc[i,3] < hp_4
        condition3 = df.iloc[i,2] > hp_3 and df.iloc[i,3] == hp_4
...
def abnormal_all_summary(df, code): #code는 유형. 1,2,3,4 입력 가능.
    ...
    prev_date = None # 이전 날짜를 저장할 변수 추가
    
    for i in range(df.shape[0]):
        current_date = df_df.iloc[i,0] 
        
        if prev_date is not None and current_date != prev_date:
            # 현재 날짜와 이전 날짜가 다를 경우 연속 카운트 초기화
            consecutive_count = 0
            start_time = None
...
def Tower_real_abnormal(df):
    ...
    if df_df.loc[0,'Code'] == 1:
        for i in range(df_df.shape[0]):
            if df_df.loc[i,'Duration'] < hp_5: #20분 이상일 때만 진짜 이상거동으로 취급
                not_real.append(i)
    if df_df.loc[0,'Code'] == 2:
        for i in range(df_df.shape[0]):
            if df_df.loc[i,'Duration'] < hp_6: #20분 이상일 때만 진짜 이상거동으로 취급
                not_real.append(i)
    if df_df.loc[0,'Code'] == 3:
        for i in range(df_df.shape[0]):
            if df_df.loc[i,'Duration'] < hp_7: #20분 이상일 때만 진짜 이상거동으로 취급
                not_real.append(i)
...
# [히트맵 그리기 4단계]: 이상 거동 히트맵 그리기
abnormal_heat_map(Tower_map_1, "Tower - abnormal behavior #type 1 (Incidence in 1h >= 20m) <heat-map>")
...
# [히트맵 그리기 4단계]: 이상 거동 히트맵 그리기
abnormal_heat_map(Tower_map_2, "Tower - abnormal behavior #type 2 (Incidence in 1h > 20m) <heat-map>")
...
# [히트맵 그리기 4단계]: 이상 거동 히트맵 그리기
abnormal_heat_map(Tower_map_3, "Tower - abnormal behavior #type 3 (Incidence in 1h > 20m) <heat-map>")    
