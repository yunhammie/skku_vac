# <<냉동기 펌프 차압 제어 루프 (제어루프 4) 이상거동 진단 코드>>
...
hp_Valve_opening = 1 # Valve_opening = 유형1의 판단 기준에 있는 밸브 개도율
hp_Res = 5 # Res = 냉수 펌프 차압 설정값 - 냉수 펌프 차압
hp_Frequency_max = 96 # Frequency_max = 냉수 펌프 인버터가 풀 가동 중일 때 주파수 기준
hp_Frequency_efficiency = 60 # Frequency_efficiency = 비효율 문제를 진단할 때 냉수 펌프 인버터의 주파수 기준
hp_Frequency_off = 0 # Frequency_off = 냉수 펌프 인버터가 꺼져있다고 판단할 때의 주파수

# 히트맵 표시 관련 하이퍼파라미터들
hp_Loop = "D" # Loop = 이상거동 진단 대상인 루프의 이름
hp_heatmap_time_type1 = 0 # heatmap_time_type1 = 이상거동 유형1의 히트맵에 표시하는 시간 기준 (1시간에 x분 이상만 표시할 때 x)
hp_heatmap_time_type2 = 15 # heatmap_time_type2 = 이상거동 유형2의 히트맵에 표시하는 시간 기준 (1시간에 x분 이상만 표시할 때 x)
hp_heatmap_time_type3 = 0 # heatmap_time_type3 = 이상거동 유형3의 히트맵에 표시하는 시간 기준 (1시간에 x분 이상만 표시할 때 x)
hp_heatmap_time_type4 = 15 # heatmap_time_type4 = 이상거동 유형4의 히트맵에 표시하는 시간 기준 (1시간에 x분 이상만 표시할 때 x)
...
def min_to_hour(df):
    ...
    for i in range(df.shape[0]):
        start_hour = int(df.iloc[i, 0].split(":")[0])
        end_hour = int(df.iloc[i, 1].split(":")[0])
...
df_4 = pd.read_csv(r"C:\Users\82107\OneDrive\BIST\5\df_chiller_on(07.31).csv", index_col=0, encoding='CP949')

# 해당 루프 이상거동 진단에 필요한 변수들 & 라벨링된 숫자들
# 0: 날짜, 1: 시간, 4: 냉수 환수/공급 온도차
# 35: 냉동기_1분간_전력사용량, 13: 냉수 유량, 22: 냉수 펌프 차압 설정값
# 23: 냉수 펌프 차압, 25: #1 냉수 펌프 주파수, 58: 밸브 개도율
Selected_Data_4 = df_4.iloc[:, [0, 1, 4, 35, 13, 22, 23, 25, 58, -2]]
...
# 밸브 개도율, 냉수 펌프 차압 설정값 각각 기울기 구하고 sample1의 열에 추가하기
# 날짜 
calendar = sample1.iloc[:, -1]

# 밸브 개도율
valve =  sample1.iloc[:, -3]
valve_incline = []
valve_incline.append(float(0))

# 냉수 펌프 차압 설정값
set_point =  sample1.iloc[:, 5]
set_point_incline = []
set_point_incline.append(float(0))

# 기울기(VOI, SPI) 구하고 sample1의 열에 추가하기
...
# 변형 데이터 - SPI & VOI(delayed by 1m)간의 correlation
valve = []
valve.append(0)
...
df_corr2["VOI(delayed by 1 m)"] = valve
...
for idx, value in sample1_1.iloc[:, 2].items():
    if (value > hp_Valve_opening or value < hp_Valve_opening * (-1)):
        if value < 0:
            idx_neg.append(idx)
        else:
            idx_post.append(idx)
...
times_4 = minute_abnormal(data_4_adapt, sample4)

# 냉수 펌프 차압 설정값과 차압의 차이에 대한 열 새로 추가하기
new_col5 = []
for i in range(0, sample4.shape[0]):
    val = float(sample4.iloc[i, 7]) - float(sample4.iloc[i, 8])
    new_col5.append(val)
...
while i < sample4.shape[0]-1:
    ...
    condition2 = sample4.iloc[i-1, 9] < hp_Res * (-1)
...
while i < sample4.shape[0]-1:
    ...
    time = sample4.iloc[i-1,1]
    condition4 = float(sample4.iloc[i-1, 10]) > hp_Frequency_max
    condition3 = float(sample4.iloc[i-1, 10]) <= hp_Frequency_max and float(sample4.iloc[i-1, 10]) > hp_Frequency_efficiency
    condition2 = float(sample4.iloc[i-1, 10]) <= hp_Frequency_efficiency and float(sample4.iloc[i-1, 10]) > hp_Frequency_off
    condition1 = float(sample4.iloc[i-1, 10]) == hp_Frequency_off
...
problems = minute_abnormal(data_4_adapt, sample4)
...
while i < sample4.shape[0]-1:
    ...
    freq_code = frequency.loc[date, time] # freq_code가 2면 냉수 펌프 인버터 풀 가동, 1이면 덜 가동
    code = 404
    print(miss_code, freq_code)
    
    if miss_code == 0: # 정상 
        code = 0
    elif miss_code == -1 and freq_code == 2: # 과한 제어이나 비효율로 분류되지는 않음
        code = 6
    elif miss_code == -1 and (freq_code == 3 or freq_code == 4): # 과한 제어, 비효율 제어
        code = 2
    else:
        if freq_code == 4:
...
# 히트맵에 1시간에 hp_heat_map_time_type2분 이상 발생한 경우만 나타내기
...
# 8-4) 유형2 - 비율 조사
# 전체 데이터 ; 44615분
sum_of_time = int(sample4.shape[0])
...
# [히트맵 그리기 4단계]: 이상 거동 히트맵 그리기
abnormal_heat_map(hour_map_4, f"loop{hp_Loop} #type 4 (Incidence in 1h >= {hp_heatmap_time_type2}m) <heat-map>")
...
print(f"loop{hp_Loop} #type 4 (Incidence in 1h >= {hp_heatmap_time_type2}m) <Incidence Rate>: " + str(round((sum_of_abnormal_4/sum_of_time)*100, 2)) + "%")
