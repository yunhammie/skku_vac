...
# df_all에서 외기냉방 중인 시간대만 가지고 있는 데이터프레임 ; df_sys_only_oac --> (9)에서 외기냉방 중일 때의 이상거동 진단 시 사용
df_sys_only_oac = df_sys.copy()
df_sys_only_oac = df_sys_only_oac.loc[idx_oac] 
df_sys_only_oac.to_csv("C:/Users/seoeu/Downloads/HVAC_Code/DATA/df_sys_on_oac_only(08.09).csv", encoding='CP949')
...
#같은 날짜에 같은 시각인 것 중복들 지우기
overlapping = []
for i in range(1, SA_df_adaptX.shape[0]):
    if SA_df_adaptX.iloc[i,0] == SA_df_adaptX.iloc[i-1,0] and SA_df_adaptX.iloc[i,1] == SA_df_adaptX.iloc[i-1,1]:
        overlapping.append(i)
SA_df_adaptX = SA_df_adaptX.drop(overlapping)
...
hp_SA_dT_normal = 0.6 # hp_SA_dT_normal: SA 온도차 정상 범위
hp_SA_setT_abnormal = 40 # hp_SA_setT_abnormal: SA 온도 설정값 이상하다 판단하는 기준
...
def SA_T_abnormal(df):
    ...
    for i in range(df.shape[0]):
        condition1 = df.iloc[i,5] < hp_SA_dT_normal*(-1) and df.iloc[i,4] == 100
        condition2 = df.iloc[i,5] > hp_SA_dT_normal and df.iloc[i,4] != 100
        condition3 = df.iloc[i,5] > hp_SA_dT_normal and df.iloc[i,4] == 100 and df.iloc[i,7] == 1
        condition4 = df.iloc[i,2] > hp_SA_setT_abnormal
...
hp_time_type1 = 5 # hp_time_type1: 이상거동 유형1 지속시간 기준
hp_time_type2 = 5 # hp_time_type2: 이상거동 유형2 지속시간 기준
hp_time_type3 = 20 # hp_time_type3: 이상거동 유형3 지속시간 기준
hp_time_type4 = 3 # hp_time_type4 이상거동 유형4 지속시간 기준
...
def SA_T_real_abnormal(df):
    ...
    if df_df.loc[0,'Code'] == 1:
        for i in range(df_df.shape[0]):
            if df_df.loc[i,'Duration'] < hp_time_type1: #5분 이상일 때만 진짜 이상거동으로 취급
                not_real.append(i)
    if df_df.loc[0,'Code'] == 2:
        for i in range(df_df.shape[0]):
            if df_df.loc[i,'Duration'] <= hp_time_type2: #5분 초과일 때만 진짜 이상거동으로 취급
                not_real.append(i)
    if df_df.loc[0,'Code'] == 3:
        for i in range(df_df.shape[0]):
            if df_df.loc[i,'Duration'] <= hp_time_type3: #20분 초과일 때만 진짜 이상거동으로 취급
                not_real.append(i)
    if df_df.loc[0,'Code'] == 4:
        for i in range(df_df.shape[0]):
            if df_df.loc[i,'Duration'] <= hp_time_type4: # 3분 초과일 때만 진짜 이상거동으로 취급
                not_real.append(i)
...
SA_T_Abnormal_1_all_grouped = min_to_hour_new(SA_T_Abnormal_1_all)
SA_T_Abnormal_2_all_grouped = min_to_hour_new(SA_T_Abnormal_2_all)
SA_T_Abnormal_3_all_grouped = min_to_hour_new(SA_T_Abnormal_3_all)
SA_T_Abnormal_4_all_grouped = min_to_hour_new(SA_T_Abnormal_4_all)
...
# [히트맵 그리기 4단계]: 이상 거동 히트맵 그리기
abnormal_heat_map(SA_T_map_1, f"loop{hp_Loop} #type 1 (Incidence in 1h >= {hp_time_type1}m) <heat-map>")
...
# [히트맵 그리기 4단계]: 이상 거동 히트맵 그리기
abnormal_heat_map(SA_T_map_2, f"loop{hp_Loop} #type 2 (Incidence in 1h > {hp_time_type2}m) <heat-map>")
...
# [히트맵 그리기 4단계]: 이상 거동 히트맵 그리기
abnormal_heat_map(SA_T_map_3, f"loop{hp_Loop} #type 3 (Incidence in 1h > {hp_time_type3}m) <heat-map>")    
...
# [히트맵 그리기 4단계]: 이상 거동 히트맵 그리기
abnormal_heat_map(SA_T_map_4, f"loop{hp_Loop} #type 4 (Incidence in 1h > {hp_time_type4}m) <heat-map>")    
...
# [히트맵 그리기 4단계]: 이상 거동 히트맵 그리기
abnormal_heat_map(SA_T_map_7, f"loop{hp_Loop} #type 7 <heat-map>")

#%% (8-1) 전수조사 히트맵 그리기

### 히트맵 그리기 - 이상거동 유형1: SA_T_map_1_all ###

# [히트맵 그리기 1단계]: 1시간 단위의 히트맵 전용 데이터프레임 df_heatmap 만들기
SA_T_map_1_all = pd.DataFrame(index=map_dates, columns=map_hour)  # 히트맵

# [히트맵 그리기 2단계]: 기존 데이터프레임 df에 데이터가 있으면, df_heatmap에 무조건 0으로 표시, 없으면 -10 표시
SA_T_map_1_all = data_exist_makr_0(SA_T_map_1_all, SA_df_adaptX)
SA_T_map_1_all = data_nonexist_mark_minus10(SA_T_map_1_all)

# [히트맵 그리기 3단계]: 이상 거동 히트맵 그리기
# (각자 dt_heatmap에 유효한 값 채우기 ; 1시간당 이상거동 유형 *번이 *분 발생하는지)
for i in range(0, SA_T_Abnormal_1_all_grouped.shape[0]):
    date = str(SA_T_Abnormal_1_all_grouped.iloc[i, 0])
    time = "{:02d}".format(SA_T_Abnormal_1_all_grouped.iloc[i, 1])
    value = SA_T_Abnormal_1_all_grouped.iloc[i, 2]
    SA_T_map_1_all.loc[date, time] = value

# [히트맵 그리기 4단계]: 이상 거동 히트맵 그리기
abnormal_heat_map(SA_T_map_1_all, f"loop{hp_Loop} #type 1 (100% investigation) <heat-map>")


### 히트맵 그리기 - 이상거동 유형2: SA_T_map_2_all ###

# [히트맵 그리기 1단계]: 1시간 단위의 히트맵 전용 데이터프레임 df_heatmap 만들기
SA_T_map_2_all = pd.DataFrame(index=map_dates, columns=map_hour)  # 히트맵

# [히트맵 그리기 2단계]: 기존 데이터프레임 df에 데이터가 있으면, df_heatmap에 무조건 0으로 표시, 없으면 -10 표시
SA_T_map_2_all = data_exist_makr_0(SA_T_map_2_all, SA_df_adaptX)
SA_T_map_2_all = data_nonexist_mark_minus10(SA_T_map_2_all)

# [히트맵 그리기 3단계]: 이상 거동 히트맵 그리기
# (각자 dt_heatmap에 유효한 값 채우기 ; 1시간당 이상거동 유형 *번이 *분 발생하는지)
for i in range(0, SA_T_Abnormal_2_all_grouped.shape[0]):
    date = str(SA_T_Abnormal_2_all_grouped.iloc[i, 0])
    time = "{:02d}".format(SA_T_Abnormal_2_all_grouped.iloc[i, 1])
    value = SA_T_Abnormal_2_all_grouped.iloc[i, 2]
    SA_T_map_2_all.loc[date, time] = value

# [히트맵 그리기 4단계]: 이상 거동 히트맵 그리기
abnormal_heat_map(SA_T_map_2_all, f"loop{hp_Loop} #type 2 (100% investigation) <heat-map>")


### 히트맵 그리기 - 이상거동 유형2: SA_T_map_3_all ###

# [히트맵 그리기 1단계]: 1시간 단위의 히트맵 전용 데이터프레임 df_heatmap 만들기
SA_T_map_3_all = pd.DataFrame(index=map_dates, columns=map_hour)  # 히트맵

# [히트맵 그리기 2단계]: 기존 데이터프레임 df에 데이터가 있으면, df_heatmap에 무조건 0으로 표시, 없으면 -10 표시
SA_T_map_3_all = data_exist_makr_0(SA_T_map_3_all, SA_df_adaptX)
SA_T_map_3_all = data_nonexist_mark_minus10(SA_T_map_3_all)

# [히트맵 그리기 3단계]: 이상 거동 히트맵 그리기
# (각자 dt_heatmap에 유효한 값 채우기 ; 1시간당 이상거동 유형 *번이 *분 발생하는지)
for i in range(0, SA_T_Abnormal_3_all_grouped.shape[0]):
    date = str(SA_T_Abnormal_3_all_grouped.iloc[i, 0])
    time = "{:02d}".format(SA_T_Abnormal_3_all_grouped.iloc[i, 1])
    value = SA_T_Abnormal_3_all_grouped.iloc[i, 2]
    SA_T_map_3_all.loc[date, time] = value

# [히트맵 그리기 4단계]: 이상 거동 히트맵 그리기
abnormal_heat_map(SA_T_map_3_all, f"loop{hp_Loop} #type 3 (100% investigation) <heat-map>")


### 히트맵 그리기 - 이상거동 유형4: SA_T_map_4_all ###

# [히트맵 그리기 1단계]: 1시간 단위의 히트맵 전용 데이터프레임 df_heatmap 만들기
SA_T_map_4_all = pd.DataFrame(index=map_dates, columns=map_hour)  # 히트맵

# [히트맵 그리기 2단계]: 기존 데이터프레임 df에 데이터가 있으면, df_heatmap에 무조건 0으로 표시, 없으면 -10 표시
SA_T_map_4_all = data_exist_makr_0(SA_T_map_4_all, SA_df_adaptX)
SA_T_map_4_all = data_nonexist_mark_minus10(SA_T_map_4_all)

# [히트맵 그리기 3단계]: 이상 거동 히트맵 그리기
# (각자 dt_heatmap에 유효한 값 채우기 ; 1시간당 이상거동 유형 *번이 *분 발생하는지)
for i in range(0, SA_T_Abnormal_4_all_grouped.shape[0]):
    date = str(SA_T_Abnormal_4_all_grouped.iloc[i, 0])
    time = "{:02d}".format(SA_T_Abnormal_4_all_grouped.iloc[i, 1])
    value = SA_T_Abnormal_4_all_grouped.iloc[i, 2]
    SA_T_map_4_all.loc[date, time] = value

# [히트맵 그리기 4단계]: 이상 거동 히트맵 그리기
abnormal_heat_map(SA_T_map_4_all, f"loop{hp_Loop} #type 4 (100% investigation) <heat-map>")
...
print(f"loop{hp_Loop} #type 1 (Incidence in 1h >= {hp_time_type1}m) <Incidence Rate>: " + str(round((sum_of_abnormal_1/sum_of_time)*100, 2)) + "%")

# 이상거동 유형 2
sum_of_abnormal_2 = int(SA_T_Abnormal_2_grouped.iloc[:, 2].sum())
print(f"loop{hp_Loop} #type 2 (Incidence in 1h > {hp_time_type2}m) <Incidence Rate>: " + str(round((sum_of_abnormal_2/sum_of_time)*100, 2)) + "%")

# 이상거동 유형 3
sum_of_abnormal_3 = int(SA_T_Abnormal_3_grouped.iloc[:, 2].sum())
print(f"loop{hp_Loop} #type 3 (Incidence in 1h > {hp_time_type3}m) <Incidence Rate>: " + str(round((sum_of_abnormal_3/sum_of_time)*100, 2)) + "%")

# 이상거동 유형 4
sum_of_abnormal_4 = int(SA_T_Abnormal_4_grouped.iloc[:, 2].sum())
print(f"loop{hp_Loop} #type 4 (Incidence in 1h > {hp_time_type4}m) <Incidence Rate>: "+ str(round((sum_of_abnormal_4/sum_of_time)*100, 2)) + "%")
...
print(f"loop{hp_Loop} #type 7 <Incidence Rate>: " + str(round((sum_of_abnormal_7/sum_of_day)*100, 2)) + "%")

#%% (8-3) 제어품질 문제 - 박스 플롯 만들어보기
...
box_plot_map(adapt_adapt, f"loop{hp_Loop} #type 7 <box-plot>")


#%% (9) 외기냉방 중일 때의 이상거동 진단 시작

# Data 불러오기 oac: 엑셀 파일 원본
oac = pd.read_csv(os.path.join(Base_dir+Data_dir +"/df_sys_on_oac_only(08.09).csv"), index_col = 0,encoding = 'CP949')
oac.reset_index(drop=True, inplace=True)

#같은 날짜에 같은 시각인 것 중복들 지우기
overlap = []
for i in range(1, oac.shape[0]):
    if oac.iloc[i,0] == oac.iloc[i-1,0] and oac.iloc[i,1] == oac.iloc[i-1,1]:
        overlap.append(i)
oac = oac.drop(overlap)

# 원하는 열 선택하기
SA_df_OAC_number = [0, 1, 42, 43, 58, 44, 97, 98, 100] 
# (0)Date,(1)Time,(2)SA설정값,(3)SA 온도,(4)밸브개도율,(5)SA 온도차,(6)SA온도 기울기,(7)냉동기 작동 여부,(8)적응기간 O/X

SA_df_OAC = oac.iloc[:, SA_df_OAC_number] # : 이상거동 진단 시 기본 데이터프레임

# 적응기간이면 제거하기
SA_df_OAC_adapt_drop = []
for i in range(SA_df_OAC.shape[0]):
    if SA_df_OAC.iloc[i,8] == 1:
        SA_df_OAC_adapt_drop.append(i)
SA_df_OAC_adaptX = SA_df_OAC.drop(SA_df_OAC_adapt_drop) # SA_T_OAC_adaptX: 이상거동 진단 시 사용하는 적응기간 제외한 데이터프레임

# 적응기간 표식열 제거
SA_df_OAC_adaptX = SA_df_OAC_adaptX.iloc[:,[0,1,2,3,4,5,6,7]]
SA_df_OAC_adaptX.reset_index(drop=True, inplace=True)

#%% (9-1) 이상거동 전수조사
# 이상거동1 - 알고리즘: SA온도차 < -0.6 일 때 밸브개도율이 100%이면 감지
# 이상거동2 - 알고리즘: SA온도차 > 0.6인데 밸브개도율이 100%가 아니면 감지
# 이상거동3 - 알고리즘: 냉동기 상관없이(외기냉방이니까) 밸브개도율 100인데 SA온도차 > 0.6이 일정 시간 지속되면 감지
# 이상거동4 - 알고리즘: SA온도 설정값(set point 오류), 밸브 개도율(센서 오류)이 상식 밖의 값을 가지고 있으면 감지

# 이상거동 유형 표시하는 열 SA_df_adaptX에 추가 (전수조사)
def SA_T_OAC_abnormal(df):
    ...
    for i in range(df.shape[0]):
        condition1 = df.iloc[i,5] < hp_SA_dT_normal*(-1) and df.iloc[i,4] == 100
        condition2 = df.iloc[i,5] > hp_SA_dT_normal and df.iloc[i,4] != 100
        condition3 = df.iloc[i,5] > hp_SA_dT_normal and df.iloc[i,4] == 100
        condition4 = df.iloc[i,2] > hp_SA_setT_abnormal
...
SA_T_OAC_Abnormal_df_all = SA_T_OAC_abnormal(SA_df_OAC_adaptX)

# 이상거동 7 - 제어품질 문제(적응기간)
# 외기냉방 시에는 적응기간 문제 따지지 X. 도메인 지식으로 봤을 때 적응기간 없음 (냉동기가 꺼져있으니까?!)

# 전수조사 요약본
SA_T_OAC_Abnormal_1_all = pd.DataFrame(abnormal_all_summary(SA_T_OAC_Abnormal_df_all, 1), columns = Columns)
SA_T_OAC_Abnormal_2_all = pd.DataFrame(abnormal_all_summary(SA_T_OAC_Abnormal_df_all, 2), columns = Columns)
SA_T_OAC_Abnormal_3_all = pd.DataFrame(abnormal_all_summary(SA_T_OAC_Abnormal_df_all, 3), columns = Columns)
SA_T_OAC_Abnormal_4_all = pd.DataFrame(abnormal_all_summary(SA_T_OAC_Abnormal_df_all, 4), columns = Columns)

#%% (9-2) 이상거동 진단 real
# 지속시간 기준 적용
# 1
if SA_T_OAC_Abnormal_1_all.shape[0] != 0:
    SA_T_OAC_Abnormal_1 = SA_T_real_abnormal(SA_T_OAC_Abnormal_1_all)
else:
    print("There is no abnormal behavior #type1")
    SA_T_OAC_Abnormal_1 = SA_T_OAC_Abnormal_1_all
# 2
if SA_T_OAC_Abnormal_2_all.shape[0] != 0:
    SA_T_OAC_Abnormal_2 = SA_T_real_abnormal(SA_T_OAC_Abnormal_2_all)
else:
    print("There is no abnormal behavior #type2")
    SA_T_OAC_Abnormal_2 = SA_T_OAC_Abnormal_2_all
# 3
if SA_T_OAC_Abnormal_3_all.shape[0] != 0:
    SA_T_OAC_Abnormal_3 = SA_T_real_abnormal(SA_T_OAC_Abnormal_3_all)
else:
    print("There is no abnormal behavior #type3")
    SA_T_OAC_Abnormal_3 = SA_T_OAC_Abnormal_3_all
# 4
if SA_T_OAC_Abnormal_4_all.shape[0] != 0:
    SA_T_OAC_Abnormal_4 = SA_T_real_abnormal(SA_T_OAC_Abnormal_4_all)
else:
    print("There is no abnormal behavior #type4")
    SA_T_OAC_Abnormal_4 = SA_T_OAC_Abnormal_4_all

# 시간 단위로 올리기
SA_T_OAC_Abnormal_1_grouped = min_to_hour_new(SA_T_OAC_Abnormal_1)
#SA_T_OAC_Abnormal_2_grouped = min_to_hour_new(SA_T_OAC_Abnormal_2)
SA_T_OAC_Abnormal_3_grouped = min_to_hour_new(SA_T_OAC_Abnormal_3)
#SA_T_OAC_Abnormal_4_grouped = min_to_hour_new(SA_T_OAC_Abnormal_4)

SA_T_OAC_Abnormal_1.set_index("Date", inplace=True)
#SA_T_OAC_Abnormal_2.set_index("Date", inplace=True)
SA_T_OAC_Abnormal_3.set_index("Date", inplace=True)
#SA_T_OAC_Abnormal_4.set_index("Date", inplace=True)

SA_T_OAC_Abnormal_1_all_grouped = min_to_hour_new(SA_T_OAC_Abnormal_1_all)
#SA_T_OAC_Abnormal_2_all_grouped = min_to_hour_new(SA_T_OAC_Abnormal_2_all)
SA_T_OAC_Abnormal_3_all_grouped = min_to_hour_new(SA_T_OAC_Abnormal_3_all)
#SA_T_OAC_Abnormal_4_all_grouped = min_to_hour_new(SA_T_OAC_Abnormal_4_all)

SA_T_OAC_Abnormal_2_grouped = SA_T_OAC_Abnormal_2
SA_T_OAC_Abnormal_4_grouped = SA_T_OAC_Abnormal_4
SA_T_OAC_Abnormal_2_all_grouped = SA_T_OAC_Abnormal_2_all
SA_T_OAC_Abnormal_4_all_grouped = SA_T_OAC_Abnormal_4_all

#%% (9-3) 외기냉방 쪽 히트맵 그리기

### 히트맵 그리기 - 이상거동 유형1: SA_T_OAC_map_1 ###
SA_T_OAC_map_1 = pd.DataFrame(index=map_dates, columns=map_hour)  # 히트맵
SA_T_OAC_map_1 = data_exist_makr_0(SA_T_OAC_map_1, SA_df_OAC_adaptX)
SA_T_OAC_map_1 = data_nonexist_mark_minus10(SA_T_OAC_map_1)
for i in range(0, SA_T_OAC_Abnormal_1_grouped.shape[0]):
    date = str(SA_T_OAC_Abnormal_1_grouped.iloc[i, 0])
    time = "{:02d}".format(SA_T_OAC_Abnormal_1_grouped.iloc[i, 1])
    value = SA_T_OAC_Abnormal_1_grouped.iloc[i, 2]
    SA_T_OAC_map_1.loc[date, time] = value
abnormal_heat_map(SA_T_OAC_map_1, f"loop{hp_Loop} only OAC #type 1 (Incidence in 1h >= {hp_time_type1}m) <heat-map>")


### 히트맵 그리기 - 이상거동 유형2: SA_T_OAC_map_2 ###
SA_T_OAC_map_2 = pd.DataFrame(index=map_dates, columns=map_hour)  # 히트맵
SA_T_OAC_map_2 = data_exist_makr_0(SA_T_OAC_map_2, SA_df_OAC_adaptX)
SA_T_OAC_map_2 = data_nonexist_mark_minus10(SA_T_OAC_map_2)
for i in range(0, SA_T_OAC_Abnormal_2_grouped.shape[0]):
    date = str(SA_T_OAC_Abnormal_2_grouped.iloc[i, 0])
    time = "{:02d}".format(SA_T_OAC_Abnormal_2_grouped.iloc[i, 1])
    value = SA_T_OAC_Abnormal_2_grouped.iloc[i, 2]
    SA_T_OAC_map_2.loc[date, time] = value
abnormal_heat_map(SA_T_OAC_map_2, f"loop{hp_Loop} only OAC #type 2 (Incidence in 1h > {hp_time_type2}m) <heat-map>")


### 히트맵 그리기 - 이상거동 유형3: SA_T_OAC_map_3 ###
SA_T_OAC_map_3 = pd.DataFrame(index=map_dates, columns=map_hour)  # 히트맵
SA_T_OAC_map_3 = data_exist_makr_0(SA_T_OAC_map_3, SA_df_OAC_adaptX)
SA_T_OAC_map_3 = data_nonexist_mark_minus10(SA_T_OAC_map_3)
for i in range(0, SA_T_OAC_Abnormal_3_grouped.shape[0]):
    date = str(SA_T_OAC_Abnormal_3_grouped.iloc[i, 0])
    time = "{:02d}".format(SA_T_OAC_Abnormal_3_grouped.iloc[i, 1])
    value = SA_T_OAC_Abnormal_3_grouped.iloc[i, 2]
    SA_T_OAC_map_3.loc[date, time] = value
abnormal_heat_map(SA_T_OAC_map_3, f"loop{hp_Loop} only OAC #type 3 (Incidence in 1h > {hp_time_type3}m) <heat-map>")


### 히트맵 그리기 - 이상거동 유형4: SA_T_OAC_map_4 ###
SA_T_OAC_map_4 = pd.DataFrame(index=map_dates, columns=map_hour)  # 히트맵
SA_T_OAC_map_4 = data_exist_makr_0(SA_T_OAC_map_4, SA_df_OAC_adaptX)
SA_T_OAC_map_4 = data_nonexist_mark_minus10(SA_T_OAC_map_4)
for i in range(0, SA_T_OAC_Abnormal_4_grouped.shape[0]):
    date = str(SA_T_OAC_Abnormal_4_grouped.iloc[i, 0])
    time = "{:02d}".format(SA_T_OAC_Abnormal_4_grouped.iloc[i, 1])
    value = SA_T_OAC_Abnormal_4_grouped.iloc[i, 2]
    SA_T_OAC_map_4.loc[date, time] = value
abnormal_heat_map(SA_T_OAC_map_4, f"loop{hp_Loop} only OAC #type 4 (Incidence in 1h > {hp_time_type4}m) <heat-map>") 

#%% (9-4) 전수조사 히트맵 그리기

### 히트맵 그리기 - 이상거동 유형1: SA_T_OAC_map_1_all ###
SA_T_OAC_map_1_all = pd.DataFrame(index=map_dates, columns=map_hour)  # 히트맵
SA_T_OAC_map_1_all = data_exist_makr_0(SA_T_OAC_map_1_all, SA_df_OAC_adaptX)
SA_T_OAC_map_1_all = data_nonexist_mark_minus10(SA_T_OAC_map_1_all)
for i in range(0, SA_T_OAC_Abnormal_1_all_grouped.shape[0]):
    date = str(SA_T_OAC_Abnormal_1_all_grouped.iloc[i, 0])
    time = "{:02d}".format(SA_T_OAC_Abnormal_1_all_grouped.iloc[i, 1])
    value = SA_T_OAC_Abnormal_1_all_grouped.iloc[i, 2]
    SA_T_OAC_map_1_all.loc[date, time] = value
abnormal_heat_map(SA_T_OAC_map_1_all, f"loop{hp_Loop} only OAC #type 1 (100% investigation) <heat-map>")


### 히트맵 그리기 - 이상거동 유형2: SA_T_map_2 ###
SA_T_OAC_map_2_all = pd.DataFrame(index=map_dates, columns=map_hour)  # 히트맵
SA_T_OAC_map_2_all = data_exist_makr_0(SA_T_OAC_map_2_all, SA_df_OAC_adaptX)
SA_T_OAC_map_2_all = data_nonexist_mark_minus10(SA_T_OAC_map_2_all)
for i in range(0, SA_T_OAC_Abnormal_2_all_grouped.shape[0]):
    date = str(SA_T_OAC_Abnormal_2_all_grouped.iloc[i, 0])
    time = "{:02d}".format(SA_T_OAC_Abnormal_2_all_grouped.iloc[i, 1])
    value = SA_T_OAC_Abnormal_2_all_grouped.iloc[i, 2]
    SA_T_OAC_map_2_all.loc[date, time] = value
abnormal_heat_map(SA_T_OAC_map_2_all, f"loop{hp_Loop} only OAC #type 2 (100% investigation) <heat-map>")


### 히트맵 그리기 - 이상거동 유형3: SA_T_map_3 ###
SA_T_OAC_map_3_all = pd.DataFrame(index=map_dates, columns=map_hour)  # 히트맵
SA_T_OAC_map_3_all = data_exist_makr_0(SA_T_OAC_map_3_all, SA_df_OAC_adaptX)
SA_T_OAC_map_3_all = data_nonexist_mark_minus10(SA_T_OAC_map_3_all)
for i in range(0, SA_T_OAC_Abnormal_3_all_grouped.shape[0]):
    date = str(SA_T_OAC_Abnormal_3_all_grouped.iloc[i, 0])
    time = "{:02d}".format(SA_T_OAC_Abnormal_3_all_grouped.iloc[i, 1])
    value = SA_T_OAC_Abnormal_3_all_grouped.iloc[i, 2]
    SA_T_OAC_map_3_all.loc[date, time] = value
abnormal_heat_map(SA_T_OAC_map_3_all, f"loop{hp_Loop} only OAC #type 3 (100% investigation) <heat-map>")


### 히트맵 그리기 - 이상거동 유형4: SA_T_map_4 ###
SA_T_OAC_map_4_all = pd.DataFrame(index=map_dates, columns=map_hour)  # 히트맵
SA_T_OAC_map_4_all = data_exist_makr_0(SA_T_OAC_map_4_all, SA_df_OAC_adaptX)
SA_T_OAC_map_4_all = data_nonexist_mark_minus10(SA_T_OAC_map_4_all)
for i in range(0, SA_T_OAC_Abnormal_4_all_grouped.shape[0]):
    date = str(SA_T_OAC_Abnormal_4_all_grouped.iloc[i, 0])
    time = "{:02d}".format(SA_T_OAC_Abnormal_4_all_grouped.iloc[i, 1])
    value = SA_T_OAC_Abnormal_4_all_grouped.iloc[i, 2]
    SA_T_OAC_map_4_all.loc[date, time] = value
abnormal_heat_map(SA_T_OAC_map_4_all, f"loop{hp_Loop} only OAC #type 4 (100% investigation) <heat-map>") 

#%% (9-5) 외기냉방 쪽 비율조사

# 이상거동 유형 1 
# 전체 데이터 ; 4950분
sum_of_time_OAC = int(4950)
sum_of_abnormal_1_OAC = int(SA_T_OAC_Abnormal_1_grouped.iloc[:, 2].sum())
print(f"loop{hp_Loop} #type 1 (Incidence in 1h >= {hp_time_type1}m) <Incidence Rate>: " + str(round((sum_of_abnormal_1_OAC/sum_of_time_OAC)*100, 2)) + "%")

# 이상거동 유형 2
sum_of_abnormal_2_OAC = int(SA_T_OAC_Abnormal_2_grouped.iloc[:, 2].sum())
print(f"loop{hp_Loop} #type 2 (Incidence in 1h > {hp_time_type2}m) <Incidence Rate>: " + str(round((sum_of_abnormal_2_OAC/sum_of_time_OAC)*100, 2)) + "%")

# 이상거동 유형 3
sum_of_abnormal_3_OAC = int(SA_T_OAC_Abnormal_3_grouped.iloc[:, 2].sum())
print(f"loop{hp_Loop} #type 3 (Incidence in 1h > {hp_time_type3}m) <Incidence Rate>: " + str(round((sum_of_abnormal_3_OAC/sum_of_time_OAC)*100, 2)) + "%")
